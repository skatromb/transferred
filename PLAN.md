...
- [ ] PG `enum` → `Utf8`, `citext` → `Utf8`. Both fall to `arrow.opaque` today, which hands a warehouse `bytea` where a status column should read `'cancelled'`. Measured on one real 2400-column schema: 10 enum columns, 2 `citext`, against 1 genuinely opaque `tsvector`.
  - Enum needs no decoding — the binary wire form *is* the UTF-8 label, already pinned by the `it_opaque` fixture, where `glad`/`sad` arrive as their own bytes. Dispatch is on `Kind::Enum(_)`, which `postgres-types` reports.
  - `Utf8`, not `Dictionary(_, Utf8)`, though an enum is exactly a dictionary: no destination in 0.1/0.2 reads one, and the variant set has nowhere to live, Arrow having no canonical enum extension. PG → PG therefore lands `text`, dropping the type as PostGIS drops its subtype.
  - `citext` is matched by name like PostGIS, its OID being per-database. Case-insensitivity is a property of the operators, not the bytes, so it cannot survive: a PG → PG trip lands `text`, where `email = 'Foo'` stops finding `foo`. Probe `citext_send` before writing the arm — plain text expected, unverified.
