...
- [x] Baselines, and why each: ADBC for both Postgres legs (Arrow-native, same binary COPY under a C driver — the closest thing to a like-for-like), duckdb's `postgres_scanner` for both Postgres legs (a whole engine, and the only baseline that reads in parallel), pyarrow and fastparquet for the Parquet legs. `psycopg` is out: it needs per-column type declarations for 22 columns and holds every row as Python objects, which does not fit in memory at this scale.
  - ADBC cannot ingest the fixture whole — it has no mapping from an Arrow struct to a Postgres type, so it refuses the range columns. Its write baseline projects them away and says so in its name.
  - ADBC does not autocommit: without an explicit `commit()` the ingest rolls back on connection exit while `adbc_ingest` still reports the row count.
  - duckdb writes too — the write leg was missing only because our own `attach` said `read_only`. It hits the same wall as ADBC from the other side: a column for its STRUCT needs a named composite type in Postgres, which it will not invent, so the range columns are out of reach there as well. Everything else keeps its type, `uuid` and `bytea` included, and WKB geometry lands as `bytea` rather than text.
  - At 1M rows duckdb wins both legs — 0.96s writing against our 1.60s, having already read in 9.51s against our 15.04s at 10M. It is the baseline to beat, and the one worth studying: parallel scan, and no per-row Python anywhere.
- [ ] Rerun the suite at 10M so the duckdb write leg has a number comparable to the rest. Needs ~11 GB free; a 10M run has already died at `ENOSPC` once.
- [ ] Drop `PgValue = Box<dyn ToSql>` (`crates/transferred-postgres/src/arrow_to_pg.rs:26`) from the write path. It is one heap allocation per value: a 20-column million-row load boxes 20M times, and a `sample` profile of it spends 27% of on-CPU time in malloc/free and 16% in memmove, against 3% decoding Parquet. Binary `COPY` is a length-prefixed field per value, so encoding each column straight into the buffer by its Arrow type needs no trait object.
  - This is what the measurement was for. duckdb runs the same single binary `COPY` on a single connection — verified by sampling `pg_stat_activity` during a load — and is still 1.3x faster on the same 20 columns, so the gap is ours to close and not a protocol difference. Column count is worth 0.34s of the 0.63s gap at 1M rows, PostGIS validation against duckdb's `bytea` memcpy another 0.06s.
...
- [x] One list of unsupported columns per baseline (`data.UNSUPPORTED`), driving a Postgres view, a `SLOW_ROWS`-capped view and a Parquet projection each. A baseline reads the projection it can manage and `transferred` reads the same one, so a row compares engines rather than how much schema each gave up. `create or replace view` cannot narrow a column list, so the views are dropped and recreated.
  - Every entry was earned by running the thing: connectorx panics (uncatchably, in Rust) on `daterange`, `int8range` and both PostGIS types; dlt refuses `extension<arrow.uuid>` and `extension<arrow.json>` outright when reading Parquet, though its own reader emits those Postgres types as plain strings; its CSV `COPY` cannot carry `bytea`.
