...
- Direct PG ↔ Arrow ↔ destination type mapping (no canonical vocab, no coercion engine, no user `schema=` — all deferred to 0.4). Coverage: primitives, `arrow.json`, `arrow.uuid`, ranges (expand), PG `geography`/`geometry`. Anything unmapped falls back to `arrow.opaque`.
