...
- [ ] Integration containers leak, one per test binary per run, plus an anonymous volume each — 58 containers and 4.2 GB of volumes accumulated over four days. `common.rs` holds the container in a `static OnceCell` and Rust never drops statics, so `ContainerAsync::drop` never fires.
  - `testcontainers` has no reaper in 0.27.3 or 0.28.0: cleanup is `Drop` only (`lib.rs:33`), and `watchdog` covers just SIGTERM/SIGINT/SIGQUIT, not a normal exit. Ryuk — the sidecar the Go/Java/Python/.NET ports use, which prunes by label once the client socket closes — is unimplemented in the Rust port ([#577](https://github.com/testcontainers/testcontainers-rs/issues/577) open since 2024-04, [#949](https://github.com/testcontainers/testcontainers-rs/pull/949) unmerged).
  - Wire Ryuk by hand: start `testcontainers/ryuk` with the docker socket mounted, label our containers, park the TCP connection in a `static` so it closes with the process. Delete it once #949 lands. Risk is resolving the socket path ourselves, which `testcontainers` does across six locations.
  - Reaping before the run instead is the wrong shape: it cleans up after the *previous* run, and a second `start()` in the same process would kill the first container.
