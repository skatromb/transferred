...
- [ ] Integration test: failure paths. Nothing forces `drop_staging` after a failed load or a failed swap, so the rollback in `destination.rs` is unexercised. Fold in the identifier-limit case: a target name at PG's 63-byte ceiling truncates the staging name into the target's own name, and `swap`'s `drop table if exists` then takes the target with it — assert the transfer errors and the existing table survives with its rows. Test the data-loss property, not `MAX_IDENTIFIER_BYTES` itself; asserting the constant restates the arithmetic it guards.
- [ ] Integration test: schema-qualified target (`myschema.tbl`). Every fixture is a bare name in `public`, so `Target::resolve`'s schema split and `qualify` are covered only for the one-part case.
- [ ] Add TLS
