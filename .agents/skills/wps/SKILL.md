...
1. `make wps` — runs `flake8` against `.flake8`, which selects `WPS` + `E999` and
   nothing else. Lint one file with
   `uv run --project crates/transferred-py --no-sync flake8 <path>`.
...
- Don't add exceptions to `.flake8` unless the user explicitly approves the edit. When
