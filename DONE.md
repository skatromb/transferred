...
- [x] Workload: Parquet → Parquet single-file via `transferred`, `PERF_ROW_NUM=N` env override.
...
- [x] Seed server-side (`create table … select from generate_series`), one wide table of 22 columns covering ints, floats, `numeric`, text, `citext`, enum, `bytea`, temporals, `uuid`, `jsonb`, two range types and PostGIS `geometry`/`geography`. `PERF_ROW_NUM` sets the scale, 50M by default, and `PERF_SLOW_ROW_NUM` caps the legs that move every row through Python.
...
- [x] `perf/fidelity.py` — the types each engine's Postgres target actually lands, from `format_type(atttypid, atttypmod)` rather than `information_schema`, which reports a bare `numeric` for a `numeric(12,4)` and would credit every engine with a loss none of them makes. Scale-independent, so `PERF_ROW_NUM=100000 make fidelity` answers it in a minute. The doc's fidelity table was assembled by hand before this, which is the kind of table that rots silently.
...
- [x] Cap the baselines that move every row through Python (`PERF_SLOW_ROW_NUM`, 1M). dlt on its defaults costs ~46 us a row reading and ~57 us writing; at `ROW_NUM` either leg alone would outlast the rest of the suite, and the gap it exists to show is already plain at a million.
...
## 0.1.3 — Postgres read leg

Goal: make the decoder the mirror of the encoder 0.1.2 landed, and take whatever CPU that costs the read leg. No public API change.

**Scope:**

- [x] `pg_to_arrow` mirrors `arrow_to_pg`: one `Decoding` variant per Postgres type, answering for both the column's Arrow field and the binary form of its values, so the two cannot disagree. Replaces the per-column closures.
  - Values decode straight into the column's Arrow builder, which retires the intermediate `Vec<Option<T>>` per column per batch.
  - Builders come from `make_builder` off the field's own `DataType`, so the schema a batch is validated against and the builder that fills it are one decision. That removed the fifteen-arm `finish()`, the `RangeDecoding` struct with its five builders and null tracker, and the closure factory that existed only to hand a range's two bounds independent builders.
  - `Bounds<T>` and its helpers leave `pg_range`; a range's bounds decode through the element type's own `Decoding`, so `date_range` is one line rather than a parallel implementation.
  - Restored a check `make_builder` skips: PG 15+ accepts `numeric(5,10)`, Arrow `Decimal128` does not, and `Decimal128Builder` would build a broken array from it. The limit comes from `arrow::datatypes::DECIMAL128_MAX_PRECISION` rather than a hand-written `38`.
  - `Encoder` takes a `SchemaRef` instead of cloning a whole `Schema` per transfer.
- [x] Rows come off the wire through `BinaryCopyOutStream` and `TryStreamExt::try_chunks`, and the source is 25 lines of it inline. The squashed commit says `try_chunks` went away with the closures, which held for one review round and no longer describes the code: batching by hand meant a `try_unfold` over a `Fuse`, a `Box::pin`, a row counter and a helper, all to reproduce a standard combinator.
  - `try_chunks` also settles the fuse. `BinaryCopyOutStream` ends on the COPY trailer — field count `-1`, the same marker we write in `copy_in.rs` — but keeps no state saying so, so a second poll falls through to a drained `CopyOutStream` and reports `Error::closed()` (`tokio-postgres-0.7.18/src/binary_copy.rs:87,118`). `TryChunks` holds its stream as `Fuse<IntoStream<St>>`, so that is no longer ours to remember.
  - Its cost is allocation, not memory: ten thousand `Vec`s of field offsets and ten thousand `Bytes` clones per batch, but only tens of MB held at once against a ~200 MB process, which is why peak RSS below is unchanged.
- [x] Perf, ten million rows, five interleaved passes: **read-leg client CPU 12.60s → 11.69s and wall 14.56s → 13.85s**, both best-of-five, with the per-pass bands not overlapping at any pass. Peak RSS unchanged. Fifty million agrees on direction, 63.2s → 58.7s of read-leg CPU.
  - The write leg is the control — untouched code, and its CPU bands do overlap (5.82 vs 5.73s), which puts the noise floor near 2% and the read-leg gap three to four times above it.
  - Wall barely follows CPU because one Postgres backend is the bottleneck, as 0.1.2 established. The gain is spendable only once partitions run concurrently.
- [x] `PERF_ROW_NUM` defaults to ten million, not fifty. Fifty measured worse: legs ran over a minute each, and the 14 GB they write puts the machine under enough memory pressure that `peak_rss_bytes` swings fourfold between passes of one unchanged build. Also documented that `make check` and friends install a debug wheel over the release one, which contaminated a five-pass run before it was noticed.
- [x] Considered and rejected: hand-rolling the COPY-out framing to replace `BinaryCopyOutStream`. It bought roughly 10% read-leg CPU for 215 lines of wire format plus 100 of tests, and those tests covered reassembling a row from several chunks — which the protocol rules out, since the backend sends "zero or more CopyData messages (always one per row)". The library validates the header and field count for free.
  - `copy_in.rs` stays hand-rolled for the opposite reason: `BinaryCopyInWriter` costs 91–103% more client CPU across both dataset sizes, and using it needs a placeholder type array the writer only counts.
- [x] Also rejected: preallocating the Arrow builders. `make_builder` is handed 0 even though `BATCH_ROWS` is known, and `finish` takes the buffer rather than clearing it, so every batch grows one from nothing — which looks like an oversight worth fixing. Passing the row count measured 2.4% *slower*, reproduced across two five-pass runs with the build order swapped between them, against a 1% noise floor. Growing by doubling reuses a warm recycled block; preallocating touches cold pages the allocator would otherwise have handed straight back.
- [x] Deploy 0.1.3.
